...
def check_limit(user_id: str, tool_name: str, db=None) -> tuple:
    ...
    if db is None:
        ...
        close_db = True
    try:
        limits = {"imagegen": settings.FREE_DAILY_IMAGE_GEN, "imageedit": settings.FREE_DAILY_IMAGE_EDIT, "songgen": settings.FREE_DAILY_SONG_GEN}
        daily_limit = limits.get(tool_name, 10)
        today = get_today_str()
        usage = db.query(Usage).filter(Usage.user_id == user_id, Usage.tool_name == tool_name, Usage.date == today).first()
...
def increment_usage(user_id: str, tool_name: str, db=None):
    ...
    try:
        today = get_today_str()
        result = db.query(Usage).filter(Usage.user_id == user_id, Usage.tool_name == tool_name, Usage.date == today).update({"count": Usage.count + 1})
        if result == 0:
